...
adminScene.hears(titles.getValues("BUTTON_CHANNELS"), (ctx) =>
  ctx.scene.enter("categoriesScene", { main_menu_button, type: "CHANNELS" })
);

adminScene.hears(titles.getValues("BUTTON_CHATS"), (ctx) =>
  ctx.scene.enter("categoriesScene", { main_menu_button, type: "CHATS" })
);

adminScene.hears(titles.getValues("BUTTON_BOTS"), (ctx) =>
  ctx.scene.enter("categoriesScene", { main_menu_button, type: "BOTS" })
);
...
adminScene.hears(titles.getValues("BUTTON_STATISTICS"), async (ctx) => {
  ...
  const res = await connection
    .query(
      `SELECT day.date, users_per_day, users_per_week from 
  ...
  const register_info = await connection
    .query(
      `SELECT day.date, registered_per_day, registered_per_week from 
  ...
  const { total_count, count_alive, count_active, count_aliens, count_arabs } =
    (
      await connection
        .query(
          `select count(id) total_count, sum(is_alive) count_alive, count(DATEDIFF(now(), lastUse) < 7) count_active, sum(language_code<>'ru') count_aliens, sum(is_arabic) count_arabs from channels.users`
        )
        .catch((e) => {
          console.log(e);
          ctx.replyWithTitle("DB_ERROR");
  ...
  dates.forEach((cur) => {
    const useInfo = res.find(
      (el) => moment(el.date).format("YYYY-MM-DD") === cur
    );

    const regInfo = register_info.find(
      (el) => moment(el.date).format("YYYY-MM-DD") === cur
    );

    if (useInfo?.users_per_day || regInfo?.registered_per_day)
      statStr += `<b>${moment(cur, "YYYY-MM-DD").format("DD.MM")}: ${
        useInfo?.users_per_day ?? 0
      }</b> уникальных запусков, <b>${
        regInfo?.registered_per_day ?? 0
      }</b> новых пользователей\n`;
  });

  statStr += `\n<b>За неделю: ${
    res?.[0]?.users_per_week ?? 0
  }</b> запусков, <b>${
    register_info?.[0]?.registered_per_week ?? 0
  }</b> новых пользователей`;
...
adminScene.hears(titles.getValues("BUTTON_GET_USERS"), async (ctx) => {
  const connection = await tOrmCon;
  const res = await connection.query(`SELECT id from users`).catch((e) => {
    console.log(e);
    ctx.replyWithTitle("DB_ERROR");
  });

  const usersStr = res.reduce((prev, cur, i) => {
    return prev + `${cur.id}\n`;
  }, "");

  ctx.replyWithDocument({
    source: Buffer.from(usersStr),
    filename: "users.txt",
  });
});
