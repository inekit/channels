...
const totalStr = 'Все каналы '+ store.channels.getCount();
...
.enter(async ctx => {
    ...
    if (random) {
        ...
        ctx.scene.state.temp_post = await ctx.replyWithKeyboard(
            ctx.getTitle('ITEM_CARD_CATEGORY', [cTitle, category_name ?? "Все каналы",store.channels.getCount(category_name)]), 
    ...
    ctx.scene.state.categories = store.channels.getCategoriesWithCountKbStr()
...
scene.action(/^category\-(.+)$/g, async ctx => {
    ...
    const cTitle = cNameExec?.[1] ? '@'+cNameExec[1] : link
    ctx.scene.state.temp_post = await ctx.replyWithKeyboard(ctx.getTitle('ITEM_CARD_CATEGORY', [cTitle, category_name ?? "Все каналы",store.channels.getCount(category_name)]), {name: 'item_keyboard', args: [link, category_name]})
    //ctx.scene.reenter({edit: true});
})

scene.hears(store.channels.getCategoriesWithCountStr(), async ctx=>{
...
scene.action('back_random', async ctx => {
    ...
    await ctx.editMenu(ctx.getTitle('ITEM_CARD_CATEGORY', [cTitle, "Все каналы",store.channels.getCount()]), {name: 'item_keyboard_main', args: [link]})
...
scene.hears(titles.getTitle('BUTTON_RANDOM_CHANNEL','ru'), ctx=>{
