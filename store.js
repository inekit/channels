...
const fileStore = {
  ...
  poster: editJsonFile(`poster.json`, {
    autosave: true,
  }),
};
...
const getCategories = (type) => () => {
  return Object.keys(fileStore?.[type].get() ?? {});
};
...
function SortArray(name, name2) {
  ...
  if (
    name > name2 ||
    name2?.substring(0, name.lastIndexOf(" ") === "Все каналы")
  ) {
    return 1;
  }
  return 0;
...
const importCategoryArray = (type, allType) => (category, array) => {
  if (!category) {
    return fileStore[allType].set("all", array);
  }
...
module.exports = {
  poster: {
    getNewPostId: () => {
      let id = parseInt(fileStore.poster.get("last_id")) + 1;
      fileStore.poster.set("last_id", id);
      return id;
    },
    getCodeInput: () => {
      return fileStore.poster.get("code_input");
    },
    enableCodeInput: () => {
      fileStore.poster.set("code_input", true);
    },
    disableCodeInput: () => {
      fileStore.poster.set("code_input", null);
    },
    setCode: (code) => {
      fileStore.poster.set("code", code);
    },
    getCode: () => {
      return fileStore.poster.get("code");
    },
  },
  channels: {
    ...
    getCategoriesWithCountStr: getCategoriesWithCountStr(
      "channels",
      "allChannels"
    ),
    getCategoriesWithCountKbStr: getCategoriesWithCountKbStr(
      "channels",
      "allChannels"
    ),
